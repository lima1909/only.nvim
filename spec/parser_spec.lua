...
describe("parse:", function()
	...
	describe("tags:", function()
		it("one node with no match is pending", function()
			...
			assert.are.same(1, vim.tbl_count(funcs))
			assert.are.same({ desc = "i1", row = 1, col = 1, erow = 1, name = "it" }, info(funcs[1]))
		...
		it("describe with no desc", function()
			...
			local funcs, sel = p.to_pending_with_tags(bufnr, "only")
			assert.are.same(1, vim.tbl_count(funcs))
			assert.are.same({ desc = "param", row = 1, col = 1, erow = 1, name = "describe" }, info(funcs[1]))
			assert.are.same(0, #sel)
		end)
...
describe("d1 only", function()
	...
	it("i21", function() end)
			...
			assert.are.same(1, vim.tbl_count(funcs))
			assert.are.same({ desc = "d1 only", row = 1, col = 0, erow = 4, name = "describe" }, info(funcs[1]))
			assert.are.same(0, #sel)
		end)
...
describe("d1 #only", function()
	...
	it("i21", function() end)
			...
			local funcs, sel = p.to_pending_with_tags(bufnr, "#only")
			assert.are.same(0, vim.tbl_count(funcs))
			assert.are.same(1, #sel)
			assert.are.same({ desc = "d1 #only", row = 1, col = 0, erow = 4, name = "describe" }, info(sel[1]))
		end)
...
describe("d1", function()
	...
	it("i21", function() end)
			...
			local funcs, sel = p.to_pending_with_tags(bufnr, "only")
			assert.are.same(1, vim.tbl_count(funcs))
			assert.are.same({ desc = "d1", row = 1, col = 0, erow = 4, name = "describe" }, info(funcs[1]))
			assert.are.same(0, #sel)
		end)
...
describe("d1", function()
	...
	it("i21", function() end)
			...
			local funcs, sel = p.to_pending_with_tags(bufnr, "only")
			assert.are.same(1, vim.tbl_count(funcs))
			assert.are.same({ desc = "i21", row = 3, col = 1, erow = 3, name = "it" }, info(funcs[3]))
			assert.are.same(1, #sel)
			assert.are.same({ desc = "i11 only", row = 2, col = 1, erow = 2, name = "it" }, info(sel[1]))
		end)
...
describe("d1", function()
	...
	it("i21 only", function() end)
			...
			assert.are.same({ desc = "i11 only", row = 2, col = 1, erow = 2, name = "it" }, info(sel[1]))
			assert.are.same({ desc = "i21 only", row = 3, col = 1, erow = 3, name = "it" }, info(sel[2]))
		end)
...
describe("d1", function()
  describe("d11 only", function()
	...
	it("i21", function() end)
			...
			assert.are.same(0, #funcs)
			assert.are.same(1, #sel)
			assert.are.same({ desc = "d11 only", row = 2, col = 2, erow = 5, name = "describe" }, info(sel[1]))
		end)
...
describe("d1", function()
  ...
  describe("d21", function()
	...
	it("i22 only", function() end)
			...
			local funcs, sel = p.to_pending_with_tags(bufnr, "only")
			assert.are.same(2, vim.tbl_count(funcs))
			assert.are.same({ desc = "i12", row = 4, col = 1, erow = 4, name = "it" }, info(funcs[4]))
			assert.are.same({ desc = "i21", row = 8, col = 1, erow = 8, name = "it" }, info(funcs[8]))
			assert.are.same(2, #sel)
			assert.are.same({ desc = "i11 only", row = 3, col = 1, erow = 3, name = "it" }, info(sel[1]))
			assert.are.same({ desc = "i22 only", row = 9, col = 1, erow = 9, name = "it" }, info(sel[2]))
		end)
...
describe("d11", function()
  ...
  it("i21 #only", function() end)
			...
			local funcs, sel = p.to_pending_with_tags(bufnr, "#only")
			assert.are.same(1, vim.tbl_count(funcs))
			assert.are.same({ desc = "i11", row = 3, col = 2, erow = 3, name = "it" }, info(funcs[3]))
			assert.are.same(1, #sel)
			assert.are.same({ desc = "i21 #only", row = 4, col = 2, erow = 4, name = "it" }, info(sel[1]))
		end)
	end)
	...
	describe("search_node:", function()
		...
		it("one node with match with no pending", function()
			...
			assert.are.same(0, vim.tbl_count(funcs))
			assert.are.same(1, #sel)
			assert.are.same({ desc = "i1", row = 1, col = 1, erow = 1, name = "it" }, info(sel[1]))
		end)
...
describe("d1", function()
	...
	it("i21", function() end)
			...
			local search_node = create_node_mock({ 1, 0 })
			local funcs, sel = p.to_pending_with_node(bufnr, search_node)
			assert.are.same(0, vim.tbl_count(funcs))
			assert.are.same(1, #sel)
			assert.are.same({ desc = "d1", row = 1, col = 0, erow = 4, name = "describe" }, info(sel[1]))
		end)
...
describe("d1", function()
	...
	it("i21", function() end)
			...
			local search_node = create_node_mock({ 2, 1 })
			local funcs, sel = p.to_pending_with_node(bufnr, search_node)
			assert.are.same(1, vim.tbl_count(funcs))
			assert.are.same({ desc = "i21", row = 3, col = 1, erow = 3, name = "it" }, info(funcs[3]))
			assert.are.same(1, #sel)
			assert.are.same({ desc = "i11", row = 2, col = 1, erow = 2, name = "it" }, info(sel[1]))
		end)
...
describe("d1", function()
  describe("d11", function()
			...
			assert.are.same(0, #funcs)
			assert.are.same(1, #sel)
			assert.are.same({ desc = "d11", row = 2, col = 2, erow = 3, name = "describe" }, info(sel[1]))
		end)
...
describe("d1", function()
  describe("d11", function()
	...
	it("i21", function() end)
			...
			assert.are.same(0, #funcs)
			assert.are.same(1, #sel)
			assert.are.same({ desc = "d11", row = 2, col = 2, erow = 5, name = "describe" }, info(sel[1]))
		end)
...
describe("d1", function()
  describe("d11", function()
	...
	it("i21", function() end)
			...
			local search_node = create_node_mock({ 4, 1 })
			local funcs, sel = p.to_pending_with_node(bufnr, search_node)
			assert.are.same(1, vim.tbl_count(funcs))
			assert.are.same({ desc = "i11", row = 3, col = 1, erow = 3, name = "it" }, info(funcs[3]))
			assert.are.same(1, #sel)
			assert.are.same({ desc = "i21", row = 4, col = 1, erow = 4, name = "it" }, info(sel[1]))
		end)
...
describe("d11", function()
  ...
  ohter_func(function() end)
			...
			local search_node = create_node_mock({ 3, 2 })
			local funcs, sel = p.to_pending_with_node(bufnr, search_node)
			assert.are.same(0, vim.tbl_count(funcs))
			assert.are.same(1, #sel)
			assert.are.same({ desc = "i11", row = 3, col = 2, erow = 3, name = "it" }, info(sel[1]))
		end)
	end)
